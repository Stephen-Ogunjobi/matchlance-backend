...
import { getCachedConversation, invalidateConversationCache, invalidateUserConversationsCache, } from "../../conversationCache.js";
...
export const handleSendMessage = async (socket, io, data) => {
    ...
    try {
        ...
        const { conversationId, content, messageType, fileUrl, fileName } = validationResult.data;
        ...
        try {
            ...
            const isParticipant = conversationPreCheck.participants.some((p) => p.toString() === userId.toString());
            if (!isParticipant) {
                socket.emit("error", { message: "Conversation not found" });
                return;
            }
            ...
            if (otherUserId) {
                ...
                const isInConversationRoom = recipientSocketId &&
            ...
            if (!message || !conversation) {
                throw new Error("Transaction failed to create message or update conversation");
            }
            ...
            await Promise.all(conversation.participants.map((participantId) => invalidateUserConversationsCache(participantId.toString())));
        }
        ...
        if (handleRateLimitError(socket, error, "Rate limit exceeded. You're sending messages too fast.")) {
            return;
        }
        console.log(error);
...
//# sourceMappingURL=sendMessageHandler.js.map