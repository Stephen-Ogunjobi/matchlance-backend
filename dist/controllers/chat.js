...
import { getCachedConversation, invalidateConversationCache, invalidateUserConversationsCache, } from "../utils/conversationCache.js";
export const getConversationByProposal = async (req, res) => {
    try {
        ...
        const conversation = await getCachedConversation(conversationDoc._id.toString());
        if (!conversation) {
            return res.status(404).json({
                success: false,
...
export const getChats = async (req, res) => {
    try {
        ...
        const isParticipant = conversation.participants.some((p) => {
            ...
            const participantId = "_id" in p ? p._id : p;
