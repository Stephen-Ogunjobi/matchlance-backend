...
export const postProposal = async (req, res) => {
    try {
        ...
        const { coverLetter, proposedBudget, estimatedTime, availability, portfolioLinks, questions, } = req.body;
        ...
        const attachments = req.files
            ? req.files.map((file) => `/uploads/proposal-attachments/${file.filename}`)
            : [];
        ...
        await Job.findByIdAndUpdate(jobId, { $push: { proposals: proposal._id } }, { new: true });
        ...
        try {
            await sendProposalNotificationEmail(email, clientName, jobTitle, freelancerName, proposal);
        }
        catch (emailError) {
...
export const updateProposal = async (req, res) => {
    try {
        ...
        const updatedProposal = await Proposal.findOneAndUpdate({ _id: proposalId }, updates, {
            new: true,
            runValidators: true,
        });
