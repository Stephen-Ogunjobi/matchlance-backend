...
import { getCachedMatchedJobs, setMatchedJobsCache, invalidateMatchedJobsCache, } from "../utils/jobCache.js";
import { getCachedFreelancerProfile, setFreelancerCache, } from "../utils/freelancerCache.js";
export const postFreelancerProfile = async (req, res) => {
    try {
        const userId = new mongoose.Types.ObjectId(req.params.userId);
        const { title, bio, skills, categories, experienceLevel, hourlyRate, availability, location, languages, } = req.body;
        ...
        await setFreelancerCache(newProfile.toObject());
...
export const getFreelancerProfile = async (req, res) => {
    try {
        ...
        await setFreelancerCache(profile.toObject());
...
export const updateFreelancerProfile = async (req, res) => {
    try {
        ...
        const updateProfile = await FreelancerProfile.findOneAndUpdate({ freelancerId }, updates, {
            new: true,
            runValidators: true,
        });
        // Update the cache with the new profile data
        if (updateProfile) {
            await setFreelancerCache(updateProfile.toObject());
        }
...
export const uploadProfilePicture = async (req, res) => {
    try {
        ...
        const updatedProfile = await FreelancerProfile.findOneAndUpdate({ freelancerId }, { profilePicture: profilePictureUrl }, {
            new: true,
            runValidators: true,
        });
        // Update the cache with the new profile picture
        if (updatedProfile) {
            await setFreelancerCache(updatedProfile.toObject());
        }
        return res.status(200).json({
...
export const getFreelancerMatchJobs = async (req, res) => {
    try {
        const userId = req.user?.userId;
        const freelancerId = req.params.freelancerId;
        if (!userId || !freelancerId) {
        ...
        if (page === 1 && limit === 20) {
            await setMatchedJobsCache(freelancerId, matchJobs);
        }
