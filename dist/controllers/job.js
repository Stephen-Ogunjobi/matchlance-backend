...
import { getCachedJob, getCachedJobsByClient, invalidateJobCache, invalidateClientJobsCache, } from "../utils/jobCache.js";
export const postNewJob = async (req, res) => {
    try {
        ...
        const { title, description, category, skills, budget, experienceLevel, duration, } = req.body;
...
export const acceptJobProposal = async (req, res) => {
    try {
        const userId = req.user?.userId;
        ...
        try {
            await sendProposalAcceptanceEmail(freelancer.email, freelancer.firstName, job.title, client.firstName, proposal);
        }
        catch (emailError) {
...
export const searchJobs = async (req, res) => {
    try {
        const { keyword, category, experienceLevel, page = "1", limit = "10", } = req.query;
...
export const rejectJobProposal = async (req, res) => {
    try {
        const userId = req.user?.userId;
        const proposalId = req.params.proposalId;
        if (!userId || !proposalId) {
