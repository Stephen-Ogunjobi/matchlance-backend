...
import { getCachedClientProfile, setClientCache, invalidateClientCache, } from "../utils/clientCache.js";
export const postClientProfile = async (req, res) => {
    try {
        const clientId = new mongoose.Types.ObjectId(req.params.clientId);
        const { bio, company, location, hiringPreferences, preferredCurrency } = req.body;
        ...
        await setClientCache(newProfile.toObject());
...
export const getClientProfile = async (req, res) => {
    try {
        ...
        await setClientCache(profile.toObject());
...
export const updateClientProfile = async (req, res) => {
    try {
        ...
        const updatedProfile = await ClientProfile.findOneAndUpdate({ clientId }, updates, { new: true, runValidators: true });
        if (!updatedProfile) {
            return res.status(404).json({ error: "Profile not found" });
        }
        await setClientCache(updatedProfile.toObject());
...
export const uploadClientProfilePicture = async (req, res) => {
    try {
        ...
        let completeness = 0;
        if (profile.location?.country && profile.location?.timezone)
            completeness += 20;
        ...
        if (profile.company?.name || profile.company?.industry)
            completeness += 20;
        if (profile.hiringPreferences?.categories?.length > 0)
            completeness += 20;
        const updatedProfile = await ClientProfile.findOneAndUpdate({ clientId }, { profilePicture: profilePictureUrl, profileCompleteness: completeness }, { new: true, runValidators: true });
        if (updatedProfile) {
            await setClientCache(updatedProfile.toObject());
        }
        return res.status(200).json({
