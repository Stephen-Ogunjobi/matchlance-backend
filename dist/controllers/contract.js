...
import { setContractCache, invalidateClientContractsCache, invalidateFreelancerContractsCache, invalidateJobContractCache, } from "../utils/contractCache.js";
...
export const createContract = async (req, res) => {
    try {
        ...
        const jobId = typeof conversation.jobId === "object" && conversation.jobId?._id
        ...
        const proposalId = typeof conversation.proposalId === "object" &&
        ...
        try {
            ...
            await Job.findByIdAndUpdate(jobId, { status: "in_progress" }, { session });
        ...
        try {
            ...
            if (freelancer && client) {
                await sendFreelancerHiredEmail(freelancer.email, freelancer.firstName, client.firstName, job.title, createdContract.toObject());
            }
        }
        catch (emailError) {
...
export const getContractByConversation = async (req, res) => {
    try {
        const userId = req.user?.userId;
        const conversationId = req.params.conversationId;
        if (!userId || !conversationId) {
